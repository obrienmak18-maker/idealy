...
| Onboarding | Nom, objectif, type de projet, niveau, découverte, voie, espace prêt. | Les migrations en production et la fonction `complete_my_onboarding` valident et enregistrent les données ; le flux visuel reste limité à trois étapes. | **Partiel** | Créer l’onboarding à six étapes avec reprise et validation Zod côté interface. |
