...
## Palier multi‑providers et workspace fichiers — 24 août 2026

Le registre Edge de `process-ai-request` accepte désormais `anthropic`, `deepseek`, `gemini`, `groq`, `moonshot`, `openai`, `openrouter` et `together`. Chaque provider possède une URL compatible, un nom de secret Edge et un catalogue de modèles par défaut. `IDEALY_PROVIDER_MODELS_JSON` permet de limiter le catalogue sans modifier le code. Kimi K3 est référencé sous `kimi-k3`; les paramètres spécifiques évitent d’envoyer une température incompatible. Le catalogue direct de l’interface reste désactivé par défaut via `NEXT_PUBLIC_IDEALY_DIRECT_MODEL_CATALOG` afin de préserver le parcours actuel tant que les nouveaux secrets n’ont pas été testés.

Un contrat `mission_files`/`mission_file_events` a été ajouté en migration locale avec RLS de lecture par propriétaire, interdiction des écritures client et fonction `append_mission_file_event` pour les séquences monotones/idempotentes. Le nouveau mode `workspaceStream` de `process-ai-request` demande au Builder un projet IUPS JSON, vérifie les chemins, calcule les checksums, persiste les fichiers et diffuse des événements `file_started`, `file_saved`, `validation_result` et `mission_completed`. Le flux Next convertit ces événements en `data-idealy-file-event`; l’artefact construit son arborescence à partir des fichiers réellement reçus au lieu d’afficher la liste codée en dur. Un endpoint authentifié permet de récupérer les fichiers et les événements après rafraîchissement.

Les validations locales du palier passent : `pnpm test:mission-files`, `pnpm backend:verify`, `pnpm typecheck`, `pnpm build` et `git diff --check`. Le runtime Deno de la fonction Edge n’est pas inclus dans le typecheck Next; le lint Biome global de cette fonction conserve des diagnostics legacy non bloquants hérités de `main`, mais aucune erreur de parsing n’a été détectée. La migration `mission_files` n’a pas été appliquée à Supabase distant dans ce palier et le workflow de déploiement Edge reste manuel/main-only. Tant que cette migration et la nouvelle version de `process-ai-request` ne sont pas déployées, le workspace réel ne doit pas être présenté comme opérationnel en production.
