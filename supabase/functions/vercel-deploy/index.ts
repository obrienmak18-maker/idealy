...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!schema?.project?.name || !schema?.project?.files || typeof schema.project.files !== 'object') {
      return corsResponse({ error: 'Invalid IUPS schema.' }, 400);
    }

    const files = Object.entries(schema.project.files as Record<string, unknown>);
    let totalBytes = 0;
    const vercelFiles = [];
    for (const [filePath, rawContent] of files) {
      if (typeof rawContent !== 'string' || filePath.length > 240) return corsResponse({ error: `Invalid file: ${filePath}` }, 400);
      const bytes = new TextEncoder().encode(rawContent).byteLength;
      if (bytes > MAX_FILE_BYTES) return corsResponse({ error: `File too large: ${filePath}` }, 413);
      totalBytes += bytes;
      if (totalBytes > MAX_TOTAL_BYTES) return corsResponse({ error: 'Project payload too large.' }, 413);
      vercelFiles.push({ file: filePath, data: encodeBase64(rawContent), encoding: 'base64' });
    }
    ...
    return corsResponse({ error: 'Deployment request failed.' }, 500);
  }
});
