...
export function EnergyGauge({ wayId, current, max, onUpgrade }: EnergyGaugeProps) {
  ...
  return (
    <button
      type="button"
      onClick={onUpgrade}
      aria-label={`${way.energy} : ${percentage}% restant, ${current} sur ${max}. Ouvrir les options de crédits.`}
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center justify-between gap-2 text-[10px] font-medium uppercase tracking-[0.12em] text-ink-400">
          ...
          <span className="font-mono normal-case tracking-normal text-ink-200">{percentage}%</span>
        </span>
        <span className="mt-1 block h-1 overflow-hidden rounded-full bg-white/10" aria-hidden="true">
          <motion.span
