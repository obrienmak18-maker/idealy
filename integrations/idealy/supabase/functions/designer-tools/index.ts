...
async function generateImage(
  ...
  const apiKey =
    ...
    Deno.env.get("IMAGE_GENERATION_API_KEY") ??
    "";
  const endpoint =
    Deno.env.get("IMAGE_GENERATION_API_URL") ??
  ...
  if (!response.ok) {
    throw new Error(`Image generation provider returned ${response.status}.`);
  }
  const result = payload?.data?.[0];
  if (!result) {
    throw new Error("Image generation provider returned no image.");
  }
  if (result.b64_json) {
    return { base64: result.b64_json, mimeType: "image/png", provider: model };
  }
  if (result.url) {
    return { ...(await downloadImage(result.url)), provider: model };
  }
  throw new Error(
    "Image generation provider returned an unsupported response."
  );
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      data: { user },
      error: userError,
    } = await client.auth.getUser(authHeader.slice("Bearer ".length));
    if (userError || !user) {
      return json({ error: "Unauthorized." }, 401, headers);
    }
    ...
    const body = (await req
      .json()
      .catch(() => null)) as Partial<DesignerRequest> | null;
    ...
    if (body.action === "searchImages") {
      ...
      const count =
        typeof body.count === "number" && Number.isInteger(body.count)
          ? Math.min(12, Math.max(1, body.count))
          : 4;
      if (!query || query.length > MAX_QUERY_CHARS) {
        return json(
          { error: "query must be between 1 and 160 characters." },
          400,
          headers
        );
      }
    ...
    if (body.action === "generateImage") {
      ...
      return json(
        { ...image, suggestedPath: "assets/generated-design.png" },
        200,
        headers
      );
    }
    ...
    return json(
      {
        error: error instanceof Error ? error.message : "Designer tool failed.",
      },
      502,
      headers
    );
