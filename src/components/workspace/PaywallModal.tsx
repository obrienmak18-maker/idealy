...
import { useStripe, PLANS } from '@/hooks/useStripe';
      ...
      <div className="relative w-full max-w-5xl rounded-2xl border border-white/10 bg-surface-container/90 p-8 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="relative rounded-xl border border-primary bg-surface p-6 flex flex-col shadow-[0_0_30px_rgba(76,215,246,0.15)] transform md:-translate-y-4">
            ...
            <div className="mb-4">
              <span className="text-3xl font-bold font-geist">{billingCycle === 'monthly' ? '49€' : '39€'}</span>
            ...
            <button onClick={() => createCheckoutSession('pro', billingCycle)} disabled={loading} className="w-full py-2.5 rounded-lg bg-primary text-on-primary font-bold shadow-[0_0_15px_rgba(76,215,246,0.4)] hover:bg-primary-fixed-dim transition-all disabled:opacity-60">
              {loading ? 'Opening secure checkout...' : 'Go Pro — 14-day trial'}
            </button>
          </div>
          ...
          <div className="rounded-xl border border-white/5 bg-surface-dim/50 p-6 flex flex-col">
            ...
            <div className="mb-4">
              <span className="text-3xl font-bold font-geist">{billingCycle === 'monthly' ? '129€' : '99€'}</span>
              <span className="text-ink-300 text-sm"> /mois</span>
            </div>
            ...
            <button onClick={() => createCheckoutSession('business', billingCycle)} disabled={loading} className="w-full py-2.5 rounded-lg border border-white/10 text-ink-100 hover:border-secondary hover:text-secondary transition-colors disabled:opacity-60">
              {loading ? 'Opening secure checkout...' : 'Go Business — 14-day trial'}
            </button>
          </div>

        </div>
        {error && <p role="alert" className="mt-5 text-center text-sm text-red-300">{error}</p>}
        {success && <p role="status" className="mt-3 text-center text-sm text-green-300">{success}</p>}
      </div>
    </div>
  );
}
